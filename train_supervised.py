...
from utils.utils import EarlyStopMonitor, get_neighbor_finder, MLP, BiLinearDecoder
...
from evaluation.evaluation import eval_score_prediction
...
for i in range(args.n_runs):
  ...
  decoder = BiLinearDecoder(node_features.shape[1], 5, dropout=DROP_OUT)
  decoder_optimizer = torch.optim.Adam(decoder.parameters(), lr=args.lr)
  decoder = decoder.to(device)
  ...
  for epoch in range(args.n_epoch):
    ...
    for k in range(num_batch):
      ...
      pred = decoder(source_embedding, destination_embedding)
      decoder_loss = decoder_loss_criterion(pred, labels_batch_torch).mean()
    ...
    val_auc = eval_score_prediction(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE, possible,
  ...
  if args.use_validation:
    ...
    test_auc = eval_score_prediction(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE, possible,
                                     n_neighbors=NUM_NEIGHBORS)
  else:
